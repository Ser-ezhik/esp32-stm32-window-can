...
inline bool validObjectType(uint8_t value) {
  return value <= static_cast<uint8_t>(ObjectType::DoubleDoor);
}

inline bool validCommand(uint8_t value) {
  return value <= static_cast<uint8_t>(Command::ConfigureCap1188);
}

inline bool validFault(uint8_t value) {
  return value <= static_cast<uint8_t>(Fault::ConfigStorage);
}
