"""Export JLCPCB BOM/CPL for the direct-IC four-channel controller."""
...
BOARD_PATH = PROJECT / "kicad" / "UNIVERSAL-4CH-F103RC-IC.kicad_pcb"
OUTPUT = PROJECT / "fabrication" / os.environ.get(
    "UNIVERSAL_4CH_RELEASE_VERSION", "v1.0-jlc-cart"
)

EXCLUDED = {"U1", "U2", "U3", "U4", "R240", "J280B"}

# Manufacturer selections are checked again by JLCPCB when the BOM is uploaded.
# If a selected part is unavailable, it must be left unplaced instead of delayed.
REF_PARTS = {
    ...
    "U300": ("STM32F103RCT6 LQFP-64", "C8323"),
...
def correction(reference: str) -> float:
    """Return bottom-side JLC library offsets verified by package pin 1."""
    if reference in {"CAP1", "U250", "U300"}:
        return 90.0
    if reference in {"D230", "D231", "D280", "L240", "U270", "Y300"}:
        return 180.0
    return 0.0
...
for fp in sorted(board.GetFootprints(), key=lambda item: item.GetReference()):
    ...
    if excluded:
        if ref.startswith("H") and (ref[1:].isdigit() or ref.startswith("HS")):
            continue
        reason = (
    ...
    if ref in {
        "CAP1", "D1101", "D1201", "D1301", "D1401", "D230", "D231",
        "D240", "D241", "D280", "L240", "U230", "U250", "U270",
        "U300", "Y300",
    }:
        audit.append({
            "Reference": ref,
...
for part in parts:
    groups[(part["Comment"], part["Footprint"], part["LCSC Part #"])].append(
        part["Reference"]
    )
bom = [
    {
...
write_csv(OUTPUT / "UNIVERSAL-4CH-F103RC-IC_BOM_JLCPCB_SMD_EXCEPT_VNH.csv",
          ("Comment", "Designator", "Footprint", "LCSC Part #"), bom)
write_csv(OUTPUT / "UNIVERSAL-4CH-F103RC-IC_CPL_JLCPCB_SMD_EXCEPT_VNH.csv",
          ("Designator", "Mid X", "Mid Y", "Layer", "Rotation"), placements)
write_csv(OUTPUT / "UNIVERSAL-4CH-F103RC-IC_MANUAL_ASSEMBLY.csv",
          ("Reference", "QuantityPerBoard", "Part", "Footprint", "Reason"), manual)
write_csv(OUTPUT / "UNIVERSAL-4CH-F103RC-IC_ORIENTATION_AUDIT.csv",
