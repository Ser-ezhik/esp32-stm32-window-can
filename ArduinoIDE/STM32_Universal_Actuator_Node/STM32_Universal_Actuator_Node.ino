...
static_assert(sizeof(NodeConfig) == 43, "Changing NodeConfig requires an EEPROM migration");
...
LocalLink localLinks[hw::SLAVE_COUNT] = {LocalLink(uartPort1), LocalLink(uartPort2), LocalLink(uartPort3)};
...
LocalStatusPayload slaveStatus[hw::SLAVE_COUNT] = {};
LocalConfigPayload slaveConfigs[hw::SLAVE_COUNT] = {};
uint32_t slaveLastSeen[hw::SLAVE_COUNT] = {};
uint8_t slaveTxSequence[hw::SLAVE_COUNT] = {};
...
bool configStorageHealthy = true;
...
bool saveConfig() {
  ...
  config.version = CONFIG_VERSION;
  config.crc = configCrc(config);
  for (uint8_t attempt = 0; attempt < hw::EEPROM_SAVE_RETRIES; ++attempt) {
    EEPROM.put(0, config);
    NodeConfig verify = {};
    EEPROM.get(0, verify);
    if (verify.magic == CONFIG_MAGIC && verify.version == CONFIG_VERSION &&
        verify.crc == configCrc(verify) && memcmp(&verify, &config, sizeof(config)) == 0) {
      configStorageHealthy = true;
      return true;
    }
    delay(2);
  }
  configStorageHealthy = false;
  return false;
}
...
bool supportedCommand(uint8_t value) {
  ...
  switch (static_cast<Command>(value)) {
    ...
    case Command::ResetCalibration:
    case Command::ConfigureCap1188:
      return true;
    default:
      return false;
  }
...
void clearFault() {
  stopLocal();
  if (!configStorageHealthy && !saveConfig()) {
...
bool applyConfigPayload(const LocalConfigPayload &payload) {
...
void executeCommand(Command command, uint8_t flags, uint16_t argument, bool fromMaster) {
  ...
  if (command == Command::ConfigureCap1188 && isMaster()) {
    config.capEnabledMask = static_cast<uint8_t>(argument) & 0x07u;
    ++config.revision;
    if (!saveConfig()) {
      setFault(Fault::ConfigStorage);
      return;
...
void pollSlaveLink() {
  ...
  uint8_t sequence = 0;
  UartType type;
  while (processed < hw::UART_RX_BUDGET_PER_LINK &&
         localLinks[0].poll(type, sequence, payload, length)) {
    ++processed;
    if (type == UartType::Stop) {
      ...
      if (supportedCommand(command.command)) {
      ...
      if (applyConfigPayload(incoming)) {
...
void pollMasterLinks() {
  for (uint8_t slot = 0; slot < carrier.slaveCount; ++slot) {
    ...
    UartType type;
    while (processed < hw::UART_RX_BUDGET_PER_LINK &&
           localLinks[slot].poll(type, sequence, payload, length)) {
      ++processed;
      if (type == UartType::Status && length == sizeof(LocalStatusPayload)) {
        ...
        if (validFault(slaveStatus[slot].fault) &&
            slaveStatus[slot].fault != static_cast<uint8_t>(Fault::None) && fault == Fault::None) {
...
void handleProvision(const CanProvisionFrame &request) {
  ...
  if (!validCabinetId(request.cabinetId) || request.slaveCount > hw::SLAVE_COUNT ||
      !validObjectType(request.objectType)) {
...
void pollCan() {
  CanMessage message;
  uint8_t processed = 0;
  while (processed < hw::CAN_RX_BUDGET_PER_LOOP && canBus.receive(message)) {
    ++processed;
    ...
    if (!supportedCommand(command.command)) continue;
...
void initializeMaster() {
  ...
  for (uint8_t i = 0; i < hw::SLAVE_COUNT; ++i) slaveConfigs[i] = defaults;
  for (uint8_t i = 0; i < hw::SLAVE_COUNT; ++i) localLinks[i].begin(hw::LOCAL_UART_BAUD);
