...
<head>
  ...
  <style>
    .cap-config,.slide-config{margin-top:16px;border-top:1px solid var(--line);padding-top:14px}.cap-config h3,.slide-config h3{font-size:15px;margin:0 0 10px}.cap-channel-grid{display:grid;grid-template-columns:repeat(4,minmax(110px,1fr));gap:8px;margin:10px 0}.cap-channel{display:flex;align-items:center;gap:8px;min-height:40px;padding:7px 9px;border:1px solid var(--line);border-radius:6px}.cap-channel input{width:18px;height:18px}.cap-summary,.slide-summary{color:var(--muted);font-size:13px}.cap-noise{color:#ffd18a}.slide-grid{display:grid;grid-template-columns:minmax(200px,1fr) 150px 150px;gap:10px;margin:10px 0}.slide-grid label{color:var(--muted);font-size:12px}.slide-grid select,.slide-grid input{display:block;width:100%;height:40px;margin-top:5px;border:1px solid var(--line);border-radius:6px;background:#12151a;color:var(--text);padding:7px 9px}.slide-toggle{display:flex;align-items:center;gap:8px}.slide-toggle input{width:18px;height:18px}@media(max-width:800px){.cap-channel-grid{grid-template-columns:1fr 1fr}.slide-grid{grid-template-columns:1fr}}
  </style>
</head>
<body>
  ...
  const slidePhases=["ожидание","основной механизм открывается","задержка открытия","створки раздвигаются","створки сдвигаются","задержка закрытия","основной механизм закрывается"];
  ...
  function slideConfigurationHtml(o){if(o.type===0||o.slideAuxiliary)return"";const candidates=objects.filter(x=>x.id!==o.id&&x.type===0&&x.actuatorCount===2&&!x.slideEnabled&&(!x.slideAuxiliary||x.cabinetId===o.slideCabinetId)),options=candidates.map(x=>`<option value="${x.cabinetId}" ${x.cabinetId===o.slideCabinetId?'selected':''}>${esc(x.name)} · CAN ${x.cabinetId}</option>`).join("");return `<div class="slide-config"><h3>Автоматическое раздвижение</h3><label class="slide-toggle"><input type="checkbox" data-slide-enabled ${o.slideEnabled?'checked':''}><span>Использовать дополнительный двухканальный модуль</span></label><div class="slide-summary">Состояние: ${o.slideEnabled?(o.slideAuxOnline?'модуль на связи':'нет связи с модулем'):'выключено'}${o.slidePhase?` · ${slidePhases[o.slidePhase]||'выполнение'}`:''}</div><div class="slide-grid"><label>Модуль<select data-slide-aux><option value="">Не выбран</option>${options}</select></label><label>Задержка открытия, мс<input data-slide-open type="number" min="0" max="10000" step="50" value="${o.slideOpenDelayMs||300}"></label><label>Задержка закрытия, мс<input data-slide-close type="number" min="0" max="10000" step="50" value="${o.slideCloseDelayMs||300}"></label></div><button data-slide-save>Сохранить</button></div>`}
  function bindSlideConfiguration(){const save=document.querySelector("[data-slide-save]");if(!save)return;save.onclick=async()=>{const enabled=document.querySelector("[data-slide-enabled]").checked,aux=document.querySelector("[data-slide-aux]").value;if(enabled&&!aux){alert("Выберите двухканальный CAN-модуль");return}try{await post("/api/object/slide",{id:selected,enabled:enabled?1:0,auxCabinetId:aux||255,openDelayMs:document.querySelector("[data-slide-open]").value,closeDelayMs:document.querySelector("[data-slide-close]").value});await loadObjects();await loadDetail()}catch(error){const messages={slide_primary_invalid:"Этот объект нельзя сделать основной дверью",slide_aux_invalid:"Выбранный модуль должен иметь два актуатора и тип «окно»",slide_aux_in_use:"Этот модуль уже привязан к другой двери"};alert(messages[error.message]||error.message||"Настройка не сохранена")}}}
  ...
  async function command(cmd){if(busy)return;busy=true;try{await post("/api/object/command",{id:selected,command:cmd});setTimeout(loadDetail,150)}catch(error){alert(error.message==="slide_unavailable"?"Дополнительный модуль раздвижения не готов или не отвечает":error.message)}finally{busy=false}}
  ...
  const renderDetailBase=renderDetail;renderDetail=o=>{renderDetailBase(o);$("detailBody").insertAdjacentHTML("beforeend",capConfigurationHtml(o)+slideConfigurationHtml(o));bindCapConfiguration();bindSlideConfiguration()};
