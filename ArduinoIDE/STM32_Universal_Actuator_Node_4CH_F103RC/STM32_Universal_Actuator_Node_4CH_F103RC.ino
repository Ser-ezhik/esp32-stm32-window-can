...
struct NodeConfig {
  ...
  uint16_t revision;
  uint16_t maxCurrentMa[hw::ACTUATOR_COUNT];
  uint16_t zeroCurrentMa[hw::ACTUATOR_COUNT];
  uint16_t pwmOpenPermille[hw::ACTUATOR_COUNT];
  uint16_t pwmClosePermille[hw::ACTUATOR_COUNT];
  uint16_t lastOpenMs[hw::ACTUATOR_COUNT];
  uint16_t lastCloseMs[hw::ACTUATOR_COUNT];
...
NodeConfig config = {};
ActuatorRuntime actuators[hw::ACTUATOR_COUNT];
...
void setDefaultConfig() {
  ...
  for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) {
...
void stopLocal() {
  for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) stopActuator(i);
}
...
void executeCommand(Command command, uint8_t flags, uint16_t argument, bool fromMaster) {
  ...
  for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) {
    startActuator(i, direction, commandPwm(i, command));
  }
...
bool localMovementFinished() {
  for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) {
    if (actuators[i].moving || !actuators[i].finished) return false;
  }
  return true;
}

bool cabinetMovementFinished() {
  return localMovementFinished();
}
...
void applyCalibration() {
  ...
  for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) {
    target = max(target, opening ? config.lastOpenMs[i] : config.lastCloseMs[i]);
  }
  if (target == 0) {
...
void readMasterSensors() {
  ...
  constexpr bool doubleDoor = false;
...
void handleProvision(const CanProvisionFrame &request) {
  ...
  if (!validCabinetId(request.cabinetId) ||
      request.slaveCount != hw::REQUIRED_LOGICAL_SLAVE_COUNT ||
      request.objectType > static_cast<uint8_t>(ObjectType::SingleDoor)) {
...
void sendCanTelemetry() {
  ...
  CanStatusFrame status = {
    ...
    static_cast<uint8_t>(position), static_cast<uint8_t>(fault), faultActuator,
    hw::LOGICAL_SLAVE_MASK
  };
  ...
  CanActuatorFrame localSecondGroup = {};
  for (uint8_t i = 0; i < 2; ++i) {
    const uint8_t actuator = i + 2u;
    localSecondGroup.currentMa[i] = actuators[actuator].filteredCurrentMa;
    localSecondGroup.pwm[i] = static_cast<uint8_t>(actuators[actuator].pwmPermille / 4u);
    localSecondGroup.flags[i] = actuators[actuator].flags;
  }
  canBus.send(
    CAN_ACTUATORS_BASE + carrier.cabinetId * 4u + 1u,
    &localSecondGroup,
    sizeof(localSecondGroup));
...
void setup() {
  ...
  // Disable JTAG while retaining PA13/PA14 SWD for programming and recovery.
  __HAL_RCC_AFIO_CLK_ENABLE();
  __HAL_AFIO_REMAP_SWJ_NOJTAG();
#endif
  bootId = static_cast<uint8_t>(micros() ^ readUidHash());
  uidHash = readUidHash();
  boardRole = BoardRole::Master;
  initializePins();
  loadConfig();
  initializeMaster();
...
void loop() {
  ...
  if (now - lastSensorAt >= hw::SENSOR_PERIOD_MS) {
    lastSensorAt = now;
    for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) updateCurrent(i);
    readMasterSensors();
  }

  if (now - lastControlAt >= hw::CONTROL_PERIOD_MS) {
    lastControlAt = now;
    for (uint8_t i = 0; i < hw::ACTUATOR_COUNT; ++i) updateActuatorProtection(i, now);
