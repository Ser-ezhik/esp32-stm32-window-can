...
class BxCan {
 public:
  bool begin(uint32_t bitrate) {
    ...
    // Remapped CAN: PB8 RX with pull-up, PB9 TX alternate push-pull 50 MHz.
    GPIOB->CRH &= ~((0xFu << 0) | (0xFu << 4));
    GPIOB->CRH |= (0x8u << 0) | (0xBu << 4);
    GPIOB->ODR |= 1u << 8;
    AFIO->MAPR = (AFIO->MAPR & ~AFIO_MAPR_CAN_REMAP) | AFIO_MAPR_CAN_REMAP_1;
